...
class Panda(_Panda):
  ...
  async def movezr(self, distance: float, speed: float = 1.0):
    await self.movep([0.0, 0.0, distance], speed=speed, reference_frame=ReferenceFrame.RELATIVE)

  async def movepr(self, positions: np.ndarray, speed: float | list[float] = 1.0):
    await self.movep(positions, speed=speed, reference_frame=ReferenceFrame.RELATIVE)

  async def mover(self, rotations: np.ndarray, speed: float | list[float] = 1.0, reference_frame: ReferenceFrame = ReferenceFrame.GLOBAL):
    rotations = np.asarray(rotations)
    assert (rotations.ndim == 3 and rotations.shape[1:] == (3, 3)) or (rotations.ndim == 2 and rotations.shape == (3, 3)), "rotations must be a 3x3 matrix or a 3D array with 3x3 dimensions"
  ...
  async def movep(self, positions: np.ndarray, speed: float | list[float] = 1.0, reference_frame: ReferenceFrame = ReferenceFrame.GLOBAL):
    positions = np.asarray(positions)
    assert (positions.ndim == 2 and positions.shape[1] == 3) or (positions.ndim == 1 and positions.shape[0] == 3), "positions must be a 3D vector or a 2D array with 3 columns"
    if reference_frame == ReferenceFrame.RELATIVE:
  ...
  async def movex(self, pose: np.ndarray, speed: float | list[float] = 1.0, reference_frame: ReferenceFrame = ReferenceFrame.GLOBAL):
    pose = np.asarray(pose)
    assert pose.shape == (4,4) or (pose.ndim == 3 and pose.shape[1:] == (4,4)), "pose must be a 4x4 matrix or a 3D array with 4x4 dimensions"
  ...
  async def movej(self, joints: np.ndarray, speed: float | list[float] = 1.0):
    ...
    assert joints.shape == (7,) or (joints.ndim == 2 and joints.shape[1] == 7), "joints must be a 7D vector or a 2D array with 7 columns"
